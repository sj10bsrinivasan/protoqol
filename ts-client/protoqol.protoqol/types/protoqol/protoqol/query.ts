...
export class QueryClientImpl implements Query {
  ...
  Params(request: QueryParamsRequest): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("protoqol.protoqol.Query", "Params", data);
    return promise.then((data) => QueryParamsResponse.decode(new _m0.Reader(data)));
  }

  Post(request: QueryGetPostRequest): Promise<QueryGetPostResponse> {
    const data = QueryGetPostRequest.encode(request).finish();
    const promise = this.rpc.request("protoqol.protoqol.Query", "Post", data);
  ...
  PostAll(request: QueryAllPostRequest): Promise<QueryAllPostResponse> {
    ...
    const promise = this.rpc.request("protoqol.protoqol.Query", "PostAll", data);
